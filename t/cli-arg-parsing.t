...
is_deeply(
    ...
    'species has correct comparator'
);
reset_context($context);

diag(
    'primary commands + props only (check that comparators are grabbed correctly)'
);
...
dies_ok( sub { $context->parse_args(qw(show --verbose update 10)) },
    'dies on parse error' );
reset_context($context);
